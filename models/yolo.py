        ...
        for m in self.model:
            if m.i<self.backbone1depth:
                ...
                if m.f != -1:  # if not from previous layer
                    if m.type == 'models.common.Concat3'or m.type == 'models.common.Concat3fix01' or m.type == 'models.common.Concat3fix02' or m.type == 'models.common.Concat3fix03' or m.type == 'models.common.Concat3fix04'  :
                        for j in m.f:
                            if j==-1:
                                x2 = x2
                            else:
    ...
    for i, (f, n, m, args) in enumerate(d['backbone1']+d['backbone2']+d['head']):  # 遍历backbone和head的每一层
        ...
        if i < backbone1depth:
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, conv_bn_relu_maxpool, Shuffle_Block, CARAFE, Concat3, Concat3fix01, Concat3fix02, Concat3fix03, Concat3fix04]:
                c1, c2 = ch[f], args[0]
                if c2 != no:  
            ...
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                ...
                for x in f:
                    if x==-1:
                        ...
                        c2p = ch[x]
                    c22 = c22+c2p
            elif m is Concat3 or m is Concat3fix01 or m is Concat3fix02 or m is Concat3fix03 or m is Concat3fix04:  # Concat：f是所有需要拼接层的索引，则输出通道c2是所有层的和
                c22 = 0
                for x in f:
                    if x == -1:
                        c2p = ch2[x]
