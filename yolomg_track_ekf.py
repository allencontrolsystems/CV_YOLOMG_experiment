...
from guncam.misc.robot_geometry import (RobotGeometryManager,
                                        BullfrogCameraIds)
from guncam.misc.CommonState import RobotState
...
# Tracking parameters
batch_size = 1  # TODO: batch inference not supported in CenterFinderYOLO yet
simulated_fps = 30
seconds_between_frames = 1 / simulated_fps
roi_size = 720
next_roi = None
old_boxes = None
new_boxes = None
...
if __name__ == "__main__":
    ...
    robot_geometry = RobotGeometryManager()
    ekf = DroneKalmanFilter.DroneKalmanFilter(robot_geometry_manager=RobotGeometryManager(), robot_state=RobotState(robot_geometry))
        ...
        for image_file in sorted(guncam_image_path.iterdir()):
            ...
            if image_file.suffix == ".bmp":
                ...
                if len(boxes) != 0:
                    ...
                    center_x = (max_box[0] + max_box[2]) / 2
                    center_y = (max_box[1] + max_box[3]) / 2
                ...
                if labels:
                    for i in range(len(labels)):
                        if old_boxes is not None:
                            cropped_image = draw_predictions(previous_2_frame, labels[i], scores[i], old_boxes[i], curr_roi, next_roi)
                        image = draw_predictions(image_draw, labels[i], scores[i], boxes[i], curr_roi, next_roi)
